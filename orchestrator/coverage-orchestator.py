#!/usr/bin/env python3
"""
Smite Coverage Campaign Orchestrator
...
class TrialConfig:
    ...
    def trial_dir(self) -> Path:
        """Per-trial output directory: <out_dir>/<target>/<label>/trial-NN/"""
        return self.out_dir / self.target / self.label / f"trial-{self.trial_num:02d}"
...
class DashboardRenderer:
    ...
    def render(cls, state: CampaignState) -> Group:
        ...
        core_table = Table(
            title=f"Smite Orchestrator ([bold yellow]Coverage Mode[/])  [cyan]{completed}[/]/[cyan]{total}[/]"
